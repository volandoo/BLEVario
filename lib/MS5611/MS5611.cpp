...
void MS5611::begin(int sda, int scl)
{
    delay(1000);
    auto started = Wire.begin(sda, scl);
    int error = -1;
    while (error != 0)
    {
...
{
    getTemperature(); // updates temperature m_dT and m_T
    uint32_t D1 = getRawPressure();

    int64_t OFF = (int64_t)m_C[2 - 1] * 65536 + (int64_t)m_C[4 - 1] * m_dT / 128;

    int64_t SENS = (int64_t)m_C[1 - 1] * 32768 + (int64_t)m_C[3 - 1] * m_dT / 256;
    m_P = (D1 * SENS / 2097152 - OFF) / 32768;
    return m_P;
}
...
int32_t MS5611::getTemperature()
{
    ...
    //****************
    // if (abs(millis() - m_lastTime) < T_THR)
    //     return m_T;
    // m_lastTime = millis();
    //****************
    uint32_t D2;
    D2 = getRawTemperature();
    m_dT = D2 - ((uint32_t)m_C[5 - 1] * 256); // update '_dT'
    ...
    m_T = 2000 + ((int64_t)m_dT * m_C[6 - 1]) / 8388608;
    return m_T;
}
...
void MS5611::readCalibration()
{
    for (uint8_t k = 0; k < 6; k++)
    {
        ...
        m_C[k] = (uint16_t)(readnBytes(NBYTES_PROM) & 0xFFFF); // masking out two LSB
    }
}
